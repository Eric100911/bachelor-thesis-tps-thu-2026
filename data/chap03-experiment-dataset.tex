...
\begin{figure}
  ...
  \caption{CMS 采用的右手直角坐标系及基本运动学变量定义。原点位于探测器中心的名义对撞点，$x$ 轴指向 LHC 环心，$y$ 轴竖直向上，$z$ 轴沿逆时针束流方向。方位角 $\phi$ 在 $x$--$y$ 横平面内从 $x$ 轴量起，极角 $\theta$ 从正 $z$ 轴量起。赝快度定义为 $\eta = -\ln[\tan(\theta/2)]$，横动量 $p_{\mathrm{T}}$ 为动量在横平面内的投影。图中标出了桶部（$|\eta| \lesssim 1.4$）和端盖区域的典型覆盖范围。~\cite{ascioti_2022_6fw3g-xj153}}
...
\begin{figure}
  \centering
  \subcaptionbox{CMS 径迹探测器整体示意图。红色线表示单面模块，蓝色线表示提供立体测量的双面模块。}
    {\includegraphics[width=0.85\textwidth]{figures/chap03-experiment-dataset/CMS-TRK-17-001_Figure_001-a.png}}\\[6pt]
  \subcaptionbox{束流管和 BPIX 像素探测器区域的放大视图，标注了像素探测器支撑管、BPIX 内外屏蔽层、三层 BPIX 探测器和束流管。}
    {\includegraphics[width=0.85\textwidth]{figures/chap03-experiment-dataset/CMS-TRK-17-001_Figure_001-b.png}}
  \caption{CMS 硅径迹探测器结构图。（上图）CMS 径迹探测器的结构示意图；（下图）原始 BPIX 探测器区域的放大视图，标注了像素探测器支撑管、BPIX 内外屏蔽层、三层 BPIX 探测器和束流管。~\cite{CMS_TRK_17_001}}
...
本分析的数据样本由 CMS 在 2022 至 2025 年 LHC 质子--质子对撞运行期间采集，质心系能量 $\sqrt{s}=13.6$~TeV，覆盖 Run~3 的全部四个数据采集年份。事例选自 \texttt{ParkingDoubleMuonLowMass} 数据流~\cite{ascioti_2022_6fw3g-xj153}，该数据流专为低质量双缪子和多缪子信号设计，适用于含 $J/\psi \to \mu^+\mu^-$ 衰变的末态。数据集的总积分亮度为 $289.21 \pm 5.97~\text{fb}^{-1}$（2022--2025）~\cite{CMS:LUMI-PUB}，累计亮度曲线和堆叠分布分别见图~\ref{fig:chap03-luminosity-run3} 和图~\ref{fig:chap03-pileup-run3}。以下按数据流与触发、取数期与重建版本、数据质量认证、积分亮度、分析输入格式五个类别分别说明。

\subsection*{title}{数据流与触发路径}
分析使用 \texttt{ParkingDoubleMuonLowMass0} 至 \texttt{ParkingDoubleMuonLowMass7} 共 8 条并行数据流，覆盖 2022--2025 年全部有效取数期。该数据流由 CMS 数据处理策略中的驻留（Parking）数据流产生：HLT 通过的事例被分流至磁带存储，不占用快速再处理资源，适合低阈值多缪子触发的高速率环境。主要触发路径为 \texttt{HLT\_Dimuon0\_Jpsi3p5\_Muon2\_v*}，针对 $J/\psi \to \mu^+\mu^-$ 候选。\textbf{[待填：最终 HLT 版本与触发菜单来源]}。

\paragraph{取数期与数据重建版本}
数据覆盖 2022 至 2025 年的所有有效取数期。各取数期的 PromptReco 重建版本汇总于表~\ref{tab:dataset-periods}。所有样本均以 MINIAOD 格式存储，每个取数期下含 8 条并行数据流（\texttt{LowMass0}--\texttt{LowMass7}）。2022 年数据集按取数期分为三个子列表：2022C--D、2022E 和 2022F--G，对应不同的数据集目录。

\begin{table}[htbp]
  \centering
  \caption{各数据采集年份的取数期与 PromptReco 重建版本。所有样本均来自 \texttt{/ParkingDoubleMuonLowMass*/.../MINIAOD} 数据集。}
  \label{tab:dataset-periods}
  \begin{tabular}{cll}
    \toprule
    年份 & 取数期 & PromptReco 版本 \\
    \midrule
    2022 & C, D, E, F, G & v1（全部），v2（仅 D 取数期） \\
    2023 & C, D & v1--v4（C 取数期），v1, v2（D 取数期） \\
    2024 & C, D, E, F, G, H, I & v1（全部），v2（E, I 取数期） \\
    2025 & C, D, E, F, G & v1（全部），v2（C, F 取数期） \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{数据质量认证}
仅保留通过 CMS Muon 数据质量标志的事例。该标志确保缪子探测器子系统处于正常运行状态，对含 $J/\psi \to \mu^+\mu^-$ 的分析至关重要。各年份对应的 Golden JSON 认证文件列于表~\ref{tab:golden-json}。这些 JSON 文件由 CMS 数据质量监测（DQM）团队根据子探测器状态逐运行（run-by-run）认证，仅包含所有必要探测器组件均处于正常运行状态的亮度段（lumisection），可从 CMS DQM 认证页面 \url{https://cms-certif.cern.ch/cms-certif/} 获取。

\begin{table}[htbp]
  \centering
  \caption{各年份的 Muon 数据质量 Golden JSON 认证文件。}
  \label{tab:golden-json}
  \begin{tabular}{cl}
    \toprule
    年份 & Golden JSON 文件 \\
    \midrule
    2022 & \texttt{Cert\_Collisions2022\_355100\_362760\_Muon.json} \\
    2023 & \texttt{Cert\_Collisions2023\_366442\_370790\_Muon.json} \\
    2024 & \texttt{Cert\_Collisions2024\_378981\_386951\_Muon.json} \\
    2025 & \texttt{Cert\_Collisions2025\_391658\_398903\_Muon.json} \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{积分亮度}
应用 Muon 数据质量筛选后，数据集的总积分亮度为 $289.21 \pm 5.97~\text{fb}^{-1}$（2022--2025）~\cite{CMS:LUMI-PUB}。\textbf{[待填：最终逐年亮度分解]}。

\paragraph{分析输入格式}
所有样本以 MINIAOD 格式提供，分析中使用的 ntuple 由自定义 skim 流程从 MINIAOD 产生。\textbf{[待填：skim 或 ntuple 产生标签和来源]}。
